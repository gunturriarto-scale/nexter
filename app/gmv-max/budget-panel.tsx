      ...
      {relevant.map((c) => {
        ...
        return (
          <div key={c.campaignId} className="gfx-card p-4">
            <p className="font-semibold text-[#14213D]">{c.campaignName}</p>
            {c.autoBudget.enabled && (
              <div className="mt-2">
                <div className="flex justify-between text-xs text-[#7A8AA3]">
                ...
                <div className="mt-1 h-2 w-full overflow-hidden rounded-none bg-[#EDF3F8]">
                  <div
                    className="h-full rounded-none bg-gradient-to-r from-[#2563EB] to-[#0891B2]"
                    style={{ width: `${pct}%` }}
                  />
                </div>
                <p className="mt-1 text-xs text-[#7A8AA3]">
            ...
            {c.promotionDays?.enabled && (
              <p className="mt-2 rounded-none bg-[#EEF4FF] px-2 py-1.5 text-xs font-medium text-[#0891B2]">
