...
export function MarketSizeBanner({ view }: { view: MarketSizeView }) {
  ...
  return (
    <div className="gfx-card gfx-gradient-soft p-6 ring-1 ring-[#f6a7bc]/40">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wider text-[#8154b6]">
            Posisi pasar Glow FX
          </div>
          <div className="mt-2 font-serif text-4xl font-semibold text-[#342d32]">
            {formatPct(view.glowShare)} <span className="text-2xl text-[#9d8a97]">dari total market</span>
          </div>
          <p className="mt-2 text-sm text-[#6b5a66]">
            Market skincare ID yang dipantau senilai{" "}
            <span className="font-semibold text-[#342d32]">{formatIdrCompact(view.totalMarket)}</span>{" "}
            ...
            <span className="font-semibold text-[#f0466d]">#{view.glowShareRank}</span> dari 6 brand
            yang dipantau.
          </p>
        </div>
        <div className="hidden shrink-0 text-right sm:block">
          <div className="text-xs text-[#9d8a97]">Leader share</div>
          <div className="font-serif text-2xl font-semibold text-[#8154b6]">
